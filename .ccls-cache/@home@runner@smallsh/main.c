...
void get_and_split_input(char *input_array[], struct sigaction sigint_function, struct sigaction sigint_action) {
  char *line = NULL;
  char *delimiter;
  char *token;
  size_t num_args = 0;
  ...
  //no line read, exit
  if (line_length == -1 || line_length == 0) goto exit;
...
void exec_input(char *exec_array[], int *bg_proc, char input_file[], char output_file[], int *bg_pid, int *bg_proc_status, int *fg_proc_status, int *bg_counter, struct sigaction sigint_action, struct sigaction sigtstp_action) {
  ...
  sigaction(SIGTSTP, &SIGTSTP_action, &SIGTSTP_default);
    ...
    if (bg_counter > 0){
      check_background(bg_pid, bg_proc_status, &bg_counter);
		...
		get_and_split_input(input, SIGINT_function, SIGINT_action);
    //sigaction(SIGINT, &SIGINT_action, NULL);
    ...
    if (*input){
      exec_input(input, &bg_proc, input_file, output_file, &bg_pid, &bg_proc_status, &fg_proc_status, &bg_counter, SIGINT_default, SIGTSTP_default);
    }
    sigaction(SIGINT, &SIGINT_action, &SIGINT_default);
    sigaction(SIGTSTP, &SIGTSTP_action, &SIGTSTP_default);
