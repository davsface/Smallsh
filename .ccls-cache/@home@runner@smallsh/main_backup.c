...
void get_and_split_input(char *input_array[]) {
  ...
  //flush output, print prompt string and tokenize user input
  //fflush(stdout);
...
void exec_input(char *exec_array[], int *bg_proc, char input_file[], char output_file[]) {
...
int main(void) {
  ...
  for (int i=0; i<WORDS; i++) {
    ...
    if (*input){
      exec_input(input, &bg_proc, input_file, output_file);
    }
