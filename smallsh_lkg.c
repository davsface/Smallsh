...
// Function to replace a string with another string within the given haystack. Source: Ryan Gambord via https://www.youtube.com/watch?v=-3ty5W_6-IQ
...
void get_and_split_input(char *input_array[], struct sigaction sigint_function, struct sigaction sigint_action, int *fg_proc_status) {
  char *line = NULL;
  char *delimiter;
  char *token;
  size_t num_args = 0;
  ...
  sigaction(SIGINT, &sigint_function, &sigint_action);
  ...
  if (line_length == -1 || line_length == 0) {
    goto exit;
  }
...
void exec_input(char *exec_array[], int *bg_proc, char input_file[], char output_file[], int *bg_pid, int *bg_proc_status, int *fg_proc_status, int *bg_counter, struct sigaction sigint_default, struct sigaction sigtstp_default, struct sigaction sig_action) {
  ...
  if (strcmp(exec_array[0], "exit") == 0){
    ...
    case 0:
			sigaction(SIGINT, &sigint_default, &sig_action);
      ...
      if (strlen(output_file) > 0) {
        ...
        if (output == -1) {
		  ...
		  if (execvp(exec_array[0], exec_array) == -1) {
        fprintf(stderr, "Error: failed to execute command.\n");
      }
		  clearerr(stdin);
      errno = 0;
      sigaction(SIGINT, &sig_action, &sigint_default);
      sigaction(SIGTSTP, &sig_action, &sigtstp_default);
      exit(2);
      break;
...
int main(void) {
  ...
  struct sigaction SIGINT_function = {0};
	SIG_action.sa_handler = SIG_IGN;
  SIGINT_function.sa_handler = handle_SIGINT;
  ...
  sigaction(SIGTSTP, &SIG_action, &SIGTSTP_default);
    ...
    if (bg_counter > 0){
      check_background(bg_pid, bg_proc_status, &bg_counter);
		...
		get_and_split_input(input, SIGINT_function, SIG_action, &fg_proc_status);
    //sigaction(SIGINT, &SIGINT_action, NULL);
    ...
    if (*input){
      exec_input(input, &bg_proc, input_file, output_file, &bg_pid, &bg_proc_status, &fg_proc_status, &bg_counter, SIGINT_default, SIGTSTP_default, SIG_action);
    }
